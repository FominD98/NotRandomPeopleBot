...
public class CommandHandler
{
    ...
    public async Task HandleCommandAsync(ITelegramBotClient botClient, TelegramMessage message)
    {
        ...
        // Handle heritage command - route only from registry objects
        if (command == "/heritage")
        {
            await HandleHeritageCommand(botClient, message);
            return;
        }
    ...
    private string GetStartMessage(string? userName, LocalizedStrings strings)
    {
        ...
        return greeting + "\n\n" +
               ...
               "🏛️ /heritage - Маршрут по объектам культурного наследия\n\n" +
    ...
    private string GetHelpMessage(LocalizedStrings strings)
    {
        return "📖 Доступные команды:\n\n" +
               ...
               "/heritage - Маршрут ТОЛЬКО по объектам культурного наследия из реестра\n" +
               "/start - Главное меню\n\n" +
               "💬 Общение:\n" +
               ...
               "/about - О боте\n\n" +
               "💡 Совет: используйте /heritage для маршрутов по историческим памятникам!";
    }
    ...
    private async Task HandleHeritageCommand(ITelegramBotClient botClient, TelegramMessage message)
    {
        var userId = message.From?.Id ?? 0;

        // Set user mode to heritage
        MessageHandler.SetUserMode(userId, "heritage");
        ...
        await botClient.SendTextMessageAsync(
            chatId: message.Chat.Id,
            text: "🏛️ Маршрут по объектам культурного наследия\n\n" +
                  "Я построю маршрут по объектам из официального реестра ОКН Республики Татарстан:\n\n" +
                  "📜 Что включено:\n" +
                  "• Памятники архитектуры\n" +
                  "• Исторические здания\n" +
                  "• Религиозные сооружения\n" +
                  "• Археологические объекты\n" +
                  "• Объекты федерального и регионального значения\n\n" +
                  "📍 Отправьте геолокацию, и я найду ближайшие объекты культурного наследия.",
            replyMarkup: keyboard
        );

        _logger.LogInformation("Sent heritage route location request to user {UserId}", userId);
    }
}
