...
public class OpenAIService : IAiService
{
    ...
    public async Task<string> GetResponseAsync(string userMessage, ConversationContext context)
    {
        try
        {
            ...
            _logger.LogInformation("Sending request to OpenAI API for user {UserId}", context.UserId);
                ...
                choices.GetArrayLength() > 0)
            {
                    ...
                    message.TryGetProperty("content", out var contentElement))
                {
                    ...
                    _logger.LogInformation("Received response from OpenAI for user {UserId}", context.UserId);
                    return aiResponse;
                }
            }

            _logger.LogWarning("Unexpected OpenAI API response format");
            return "Не удалось получить ответ от AI.";
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error calling OpenAI API for user {UserId}", context.UserId);
